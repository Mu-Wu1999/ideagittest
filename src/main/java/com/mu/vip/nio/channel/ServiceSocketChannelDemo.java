package com.mu.vip.nio.channel;
...
public class  ServiceSocketChannelDemo {
    ...
    public static class TCPEchoServer implements Runnable{
        ...
        public void run(){
            ...
            try {
                ...
                /*服务器通道只能对tcp链接事件感兴趣，一旦selector检测到注册的ssc可以被连接
                    就会把ssc对应的key添加到selected-key set*/
            ...
            try{
                ...
                while(!Thread.currentThread().isInterrupted()){
                    /*检查一组key对应的channel是不是可用*/
                    int n = selector.select();
                    if(n == 0){
                        continue;
                    }
                        /*返回selector被选中的key的集合，事件就绪，则对应事件的key就会被添加到这个集合中*/
                    ...
                    while(it.hasNext()){
                        ...
                        try{
                            ...
                            if(key.isReadable()){
                                ...
                                System.out.println(cb.array()+"测试数据有没有到这里");
                            ...
                            if(key.isWritable()){
                                ...
                                if(len != 0){
                                    /*取消通道的写事件| 就是  update 一个 read 的事件
                                                    &~  就是  取消 一个 read 事件
                                    **/
                                    key.interestOps(key.interestOps() & (~SelectionKey.OP_WRITE));
                                }
    ...
    public static void main(String[] args) throws InterruptedException, IOException{
        Thread thread = new Thread(new TCPEchoServer(8888));
        thread.start();
        Thread.sleep(100000);
        /*结束服务器线程*/
        thread.interrupt();
